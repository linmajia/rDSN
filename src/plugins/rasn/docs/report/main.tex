...
Resilience & Bounded coordinator retry, request deadlines, a per-provider
model circuit breaker (closed/open/half-open) that fast-fails failing endpoints,
a per-provider admission gate (concurrency bulkhead plus \texttt{exp\_delay}
backpressure) that protects healthy-but-slow endpoints, and a per-provider
token-bucket rate limiter that paces calls under the endpoint's
requests-per-minute quota --- the failure, concurrency, and throughput dimensions
--- surfaced through \texttt{perf\_counter} metrics and a \texttt{rasn.resilience}
command & Tool and remote-agent gateways are not yet guarded; the limits are
per-provider rather than a single process-wide budget, and govern request count
rather than token/cost budgets \\
\bottomrule
\end{tabular}
\end{table*}
...
\texttt{admission\_gate} & Resilience & Per-provider concurrency bulkhead and \texttt{exp\_delay} backpressure \\
\texttt{rate\_limiter} & Resilience & Per-provider token-bucket rate limiter (requests/minute pacing) \\
...
\begin{itemize}
  ...
  staged admission-control delay, reused as the model gateway's backpressure curve
  so overload throttling matches the host runtime's own task-queue policy.
  \item \texttt{dsn\_now\_ms} (the env-provider clock) injected into the model
  circuit breaker and token-bucket rate limiter, so cooldown and token refill are
  deterministic under replay and seedable rather than reading a private wall clock.
\end{itemize}
...
\texttt{command\_manager}. Tool and remote-agent gateways are not yet
breaker-guarded, which is noted as future work.
...
threshold to the configured ceiling. Second, the gate reuses the breaker's
\texttt{in\_process()} predicate to exempt no-network providers, and is wired into
\texttt{rasn\_llm\_agent\_service} on the same convergence point as the breaker.
Ordering is deliberate: the admission \emph{reservation} is evaluated \emph{before}
the breaker (so a rejected request never perturbs breaker state) and \emph{after}
...
without a live node, exactly as the breaker's cooldown clock is. It is wired into
\texttt{rasn\_llm\_agent\_service} beside the other two gates. Ordering is again
deliberate: admission reserves a slot and the rate limiter acquires a token first,
and the breaker --- which consumes a one-shot half-open probe that must be paired
with a result report --- is consulted \emph{last}, so a request that admission or
the rate limiter rejects never strands a half-open probe. The pacing delay, like
...
counters, and the \texttt{rasn.metrics} command. The CodePilot tool tests cover

...
model circuit breaker (closed/open/half-open), per-provider admission and rate
gates for model calls, and per-tool admission/rate gates for tool execution,
all surfaced through \texttt{perf\_counter} metrics and a
\texttt{rasn.resilience} command & Remote-agent gateways are not yet guarded;
the limits are per-dependency rather than a single process-wide budget, and
govern request count rather than token/cost budgets \\
\bottomrule
\end{tabular}
\end{table*}
...
admission/rate controls instead), and remote-agent gateway guards remain future
work.
...
the expanded \texttt{rasn.resilience} command, which now reports both model and tool
...
Overload/dependency isolation & Model providers have circuit-breaker,
admission, and rate controls; tools have per-tool admission and rate controls;
all are surfaced through \texttt{observe resilience} and
\texttt{rasn.resilience} & Remote-agent gateways are not yet guarded; limits are
per-dependency rather than process-wide and request-count based rather than
token/cost based \\
\bottomrule
\end{tabular}
\end{table*}

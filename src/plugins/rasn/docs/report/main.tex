...
model circuit/admission/rate guards, per-tool admission/rate gates,
per-remote-agent RPC circuit/admission/rate guards, and a process-wide overload
budget (global concurrency bulkhead plus request-rate ceiling), all surfaced
through \texttt{perf\_counter} metrics and a \texttt{rasn.resilience} command & The
process-wide overload budget is request-count based; per-provider token/cost
budgets are enforced separately at the model gateway \\
\bottomrule
\end{tabular}
\end{table*}
...
\texttt{rasn.resilience} command. Because a hosted API also meters tokens, not just
requests, a fourth governor reuses the same bucket as a per-provider token/cost
budget: it charges a deterministic prompt-size estimate rather than a flat $1$ per
call, so the gateway bounds tokens-per-minute and spend as well as request count.
...
compile into a \texttt{rasn} static library, and \codepilot, SREPilot, and the
\texttt{rasn.unit\_tests} executable link that library instead of recompiling
the engine. \texttt{CMakeLists.txt} integrates the library, application
subdirectories, and the unit tests into the \rdsn build. The implementation is
intentionally header-heavy for message serialization helpers and uses \cpp
service classes for runtime behavior.
...
it distinguishes reusable nucleus code from application layers such as CodePilot
and SREPilot.
...
\texttt{model\_cost} & Resilience & Deterministic prompt-size token/cost estimator mapped onto the token bucket (tokens/minute budget, saturated diagnostics for oversized estimates) \\
...
\texttt{apps/codepilot/*} & Application & CLI commands, local tools, skills, provider configuration \\
\texttt{apps/srepilot/*} & Application & Incident diagnosis, runbook, status, observability commands \\
\bottomrule
\end{tabular}
\end{table*}
...
\texttt{agent\_tools.*} and \texttt{apps/codepilot/local\_tools.*} implement local
...
process-wide concurrency budget layers above it (see the Process-Wide Overload
Budget subsection).
...
the \texttt{rasn.resilience} command.

The rate limiter governs request \emph{count}, but a hosted model API also meters
\emph{tokens} per minute, and on metered endpoints that token dimension is what
drives spend: a request counter cannot bound it because one large-context prompt
can cost many times a small one. \rasn therefore adds a fourth model-gateway
governor, a token/cost budget, that does not introduce a second engine but reuses
the token bucket, which meters an arbitrary weight. Instead of the constant $1$ a
request counter spends, the budget spends an estimated token charge
$\max(1, \lceil c / k \rceil \cdot p / 100)$ over the prompt characters $c$, with a
characters-per-token divisor $k$ and a completion-allowance percent $p$. The
estimate is a pure function of prompt size, never of the provider response, so
charging a request stays deterministic and replay-safe. \texttt{cost\_tokens\_per\_min}
becomes the bucket refill rate and \texttt{cost\_burst\_tokens} its capacity, so the
same pacing/fast-fail logic bounds tokens-per-minute and cost. The budget slots into
the guard chain immediately after the request-count rate limiter and before the
authoritative breaker check, so a cost rejection never strands a half-open probe;
its pacing joins the single coalesced wait, and refunds compose (a cost rejection
refunds the rate token, and a post-charge breaker short-circuit refunds both the
rate token and the token charge). It is disabled by default
(\texttt{cost\_tokens\_per\_min}~$=0$ means unlimited) and exempts no-network
providers, so existing behavior is unchanged. Rejections and pacing are exported as
\texttt{rasn\_model\_cost\_limited\_total} and
\texttt{rasn\_model\_cost\_delayed\_total}, and per-provider tokens/minute, burst,
and available budget are shown by \texttt{rasn.resilience}. The implementation also
hardens the arithmetic boundary exposed by operator configuration: oversized
estimated token counts saturate before they are stored in diagnostics or events,
and the shared token bucket rejects unrepresentably large projected waits before
converting them to the millisecond API type. A process-wide
request-rate ceiling still layers above the per-provider limiter (see the
Process-Wide Overload Budget subsection).
...
\subsection{Process-Wide Overload Budget}

The three preceding gateways each protect one outbound boundary, but a host has
finite \emph{shared} capacity --- worker threads, memory, sockets, file
descriptors. When many dependencies are individually healthy yet all busy at once,
the sum of their deliberately generous per-dependency caps can still exhaust the
process. \rasn therefore adds a single process-wide overload budget at the
coordinator's \texttt{invoke} choke point: one global concurrency bulkhead plus one
global request-rate ceiling that bound \emph{total} in-flight work and throughput
across every dependency (model, tool, remote-agent), in both inline and RPC modes.

The budget reuses the same \texttt{admission\_gate} and \texttt{rate\_limiter}
engines as the per-dependency gateways, but as process-wide singletons rather than
per-key registries, lazily constructed under a \texttt{std::once\_flag} from
\texttt{[rasn.overload]}. It is applied at the outermost point of \texttt{invoke}:
\emph{after} \texttt{begin\_request} and the \texttt{scoped\_agent\_request} guard
--- so a malformed or cancelled request consumes no budget, the same
preflight-before-slots principle used for remote-agent endpoint validation --- and
\emph{before} capability resolution and routing, so the whole process sheds load
uniformly. Because \texttt{invoke} is never re-entered on the same thread (inline
handlers call external providers, workflow node dispatch is a flat sequential loop,
and RPC handlers run on separate threads), one admission slot per top-level
operation counts each logical operation exactly once with no self-deadlock. The
slot is a local RAII handle held across retries and released on every return path.
Because the gate sits deliberately \emph{outside} route resolution --- in
RPC-client mode \texttt{resolve} issues a synchronous registry query, itself
dependency work the budget must bound --- the single pre-dispatch short-circuit is
a route-resolution failure, and there the rate token is \emph{refunded} before the
error returns, mirroring the model and remote-agent gateways so repeated routing
failures cannot slowly drain the global bucket. A shared
\texttt{enter\_overload\_gate} helper applies the identical
admission-then-rate-then-backpressure sequence at both entry points: \texttt{invoke}
and the inline streaming fast path in \texttt{complete\_streaming}, which dispatches
straight to the local model agent for token-by-token streaming rather than buffering
through \texttt{invoke} and would otherwise be the one way to bypass the budget.
Admission and rate delays are coalesced into
a single sleep, and rejections are structured, non-retryable \texttt{overload}
responses so a caller with alternate nodes can route away instead of hammering a
saturated process.

Unlike the per-dependency caps, the concurrency defaults are \texttt{0}
(disabled/passthrough), so zero-config behavior is unchanged and an operator opts in
by sizing a global cap to the host. Four counters ---
\texttt{rasn\_overload\_admission\_rejected\_total},
\texttt{rasn\_overload\_admission\_delayed\_total},
\texttt{rasn\_overload\_rate\_limited\_total}, and
\texttt{rasn\_overload\_rate\_delayed\_total} --- flow through the same
\texttt{record\_event} choke point, and live budget state (in-flight, configured
caps, cached tokens) \emph{heads} the \texttt{rasn.resilience} and
\texttt{observe resilience} report, since the process-wide budget governs every
per-dependency gateway below it. The per-provider model gateway now closes the
token/cost dimension with its own budget (see the Model Gateway Rate Limiter
subsection); the remaining process-wide gap is dimensional rather than structural:
the global budget still governs request count, not aggregate token/cost.
...
counters, remote-agent resilience counters, process-wide overload counters, and the
\texttt{rasn.metrics} and
...
have circuit-breaker, admission, and rate controls; the model gateway adds a
per-provider token/cost budget; tools have per-tool admission and rate controls; a
process-wide overload budget bounds total in-flight work and throughput across all
dependencies; all are surfaced through
\texttt{observe resilience} and \texttt{rasn.resilience} & The process-wide budget
is request-count based rather than token/cost based \\
\bottomrule
\end{tabular}
\end{table*}

...
and a per-provider admission gate (concurrency bulkhead plus \texttt{exp\_delay}
backpressure) that protects healthy-but-slow endpoints, surfaced through
\texttt{perf\_counter} metrics and a \texttt{rasn.resilience} command & Tool and
remote-agent gateways are not yet guarded; the cap is per-provider rather than a
single process-wide budget \\
\bottomrule
\end{tabular}
\end{table*}
...
Ordering is deliberate: admission is evaluated \emph{before} the breaker (so a
rejected request never perturbs breaker state) and \emph{after} the replay check (so
replayed runs stay deterministic), and an RAII \texttt{admission\_slot} releases
capacity on every exit, including an early breaker short-circuit. With the generous

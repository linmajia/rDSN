...
overload budget bounds request count rather than token/cost budgets \\
\bottomrule
\end{tabular}
\end{table*}
...
the \texttt{rasn.resilience} command. Rate limiting governs request count; a
process-wide request-rate ceiling layers above the per-provider limiter (see the
Process-Wide Overload Budget subsection), while token/cost budgets remain future
work.
...
per-dependency gateway below it. The remaining overload gap is dimensional rather
than structural: rate limiting still governs request count, not token/cost budgets.
...
and rate controls; a process-wide overload budget bounds total in-flight work and
throughput across all dependencies; all are surfaced through

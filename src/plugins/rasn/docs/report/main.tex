...
Resilience & Bounded coordinator retry, request deadlines, and a per-provider
...
command & Tool and remote-agent gateways are not breaker-guarded; no global
concurrency cap yet \\
\bottomrule
\end{tabular}
\end{table*}
...
replay and replayed runs and local/simulator providers bypass it entirely.
...
engine is unit-tested without a live node. Replayed runs and local/in-process
providers (the deterministic simulator) bypass the breaker, so existing behavior
and tests are unchanged. Second, breaker activity is reported through the same

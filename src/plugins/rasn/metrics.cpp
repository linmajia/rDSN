...
enum core_counter
{
    ...
    CC_MODEL_COST_LIMITED,
    CC_MODEL_COST_DELAYED,
...
const counter_def k_core_counters[CC_COUNT] = {
    ...
    {"rasn_replay_misses_total", "replay.miss", "replay determinism mismatches (recorded value/tool/order not found)"},
    {"rasn_failures_total", "failures", "classified failures"},
    {"rasn_model_breaker_open_total", "model.breaker.open", "model provider circuit-breaker open transitions"},
    ...
    {"rasn_model_rate_limited_total",
     "model.rate.limited",
     "model requests rejected by the client-side rate limiter"},
    {"rasn_model_rate_delayed_total",
     "model.rate.delayed",
     "model requests delayed (paced) by the client-side rate limiter"},
    {"rasn_model_cost_limited_total",
     "model.cost.limited",
     "model requests rejected by the token/cost budget"},
    {"rasn_model_cost_delayed_total",
     "model.cost.delayed",
     "model requests delayed (paced) by the token/cost budget"},
...
const std::unordered_map<std::string, int> &kind_to_core_counter()
{
    static const std::unordered_map<std::string, int> m = {
        ...
        {"model.cost.limited", CC_MODEL_COST_LIMITED},
        {"model.cost.delayed", CC_MODEL_COST_DELAYED},

...
std::string absolute_or_original(const std::string &path)
{
...
bool read_context_prefix(const std::string &path,
                         size_t max_bytes,
                         std::string *content,
                         bool *truncated,
                         std::string *error)
...
size_t clamp_to_size_t(uint64_t value)
{
...
size_t capped_count(uint64_t configured_limit, size_t available)
{
    return clamp_to_size_t((std::min)(configured_limit, static_cast<uint64_t>(available)));
}

uint64_t saturating_multiply(uint64_t value, uint64_t multiplier)
{
...
uint64_t workspace_candidate_scan_limit(const workspace_index_options &options)
{
...
uint64_t workspace_entry_scan_limit(const workspace_index_options &options)
{
    return (std::max)(saturating_multiply(workspace_candidate_scan_limit(options), 16), static_cast<uint64_t>(4096));
}

const std::unordered_set<std::string> &workspace_ignored_components()
{
...
const std::unordered_set<std::string> &workspace_sensitive_file_names()
{
...
const std::unordered_map<std::string, int> &workspace_file_priorities()
{
...
const std::unordered_map<std::string, int> &workspace_extension_priorities()
{
    static const std::unordered_map<std::string, int> priorities = {
...
bool workspace_extension_is_source_code(const std::string &ext)
{
...
bool path_component_is_ignored(const std::string &previous_component, const std::string &component)
{
    if (component.empty())
    {
        return false;
...
bool path_has_ignored_component(const std::string &path)
{
...
bool is_workspace_source_candidate(const std::string &path)
{
    const std::string file_name = lower_ascii(::dsn::utils::filesystem::get_file_name(path));
...
int workspace_source_priority(const std::string &path)
{
    const std::string file_name = lower_ascii(::dsn::utils::filesystem::get_file_name(path));
...
std::string relative_workspace_path(const std::string &workspace_root, const std::string &path)
{
...
bool content_looks_binary(const std::string &content)
{
...
std::string scan_child_path(const std::string &directory, const char *name)
{
    if (directory.empty())
    {
...
bool list_workspace_directory_once(const std::string &directory,
...
uint64_t file_size_bytes(const std::string &path)
{
    ...
    return size < std::streampos(0) ? 0 : static_cast<uint64_t>(size);
}

bool collect_workspace_source_candidates(const std::string &absolute,
...
void sort_workspace_candidates(const std::string &absolute, std::vector<std::string> *candidates)
{
    std::sort(candidates->begin(), candidates->end(), [&absolute](const std::string &left, const std::string &right) {
        const std::string left_relative = relative_workspace_path(absolute, left);
        const std::string right_relative = relative_workspace_path(absolute, right);
        const int left_priority = workspace_source_priority(left_relative);
        const int right_priority = workspace_source_priority(right_relative);
        if (left_priority != right_priority)
        {
            return left_priority < right_priority;
        }
        const size_t left_depth = path_depth(left_relative);
        const size_t right_depth = path_depth(right_relative);
...
workspace_file_entry make_workspace_file_entry(const std::string &absolute, const std::string &file)
{
    workspace_file_entry entry;
    entry.absolute_path = file;
    entry.relative_path = relative_workspace_path(absolute, file);
    entry.size_bytes = file_size_bytes(file);
    entry.priority = workspace_source_priority(entry.relative_path);
    return entry;
}

void fill_index_from_candidates(const std::string &absolute,
                                const workspace_index_options &options,
...
{
    *index = workspace_index_result();
    index->workspace_root = absolute;
    index->matched_files = static_cast<uint64_t>(candidates.size());
    index->entries_seen = entries_seen;
    index->truncated = scan_truncated;

    const size_t files_to_list = capped_count(options.max_files, candidates.size());
    index->files.reserve(files_to_list);
    for (size_t i = 0; i < files_to_list; ++i)
    {
        index->files.push_back(make_workspace_file_entry(absolute, candidates[i]));
    }
                           ...
                           std::string *error)
{
    ...
    const std::string absolute = absolute_or_original(workspace_root);
    std::vector<std::string> candidates;
    uint64_t entries_seen = 0;
    bool scan_truncated = false;
    if (!collect_workspace_source_candidates(absolute, options, &candidates, &entries_seen, &scan_truncated, error))
    {
        return false;
    }
    sort_workspace_candidates(absolute, &candidates);
    fill_index_from_candidates(absolute, options, candidates, entries_seen, scan_truncated, index);
    return true;
}
                                      ...
                                      std::string *error)
{
    ...
    *snapshot = workspace_context_snapshot();
    ...
    sort_workspace_candidates(absolute, &candidates);
    fill_index_from_candidates(absolute, options, candidates, entries_seen, scan_truncated, &snapshot->index);
    snapshot->truncated = snapshot->index.truncated;
    ...
    if (snapshot->index.files.size() < candidates.size())
    {
        output << " (first " << snapshot->index.files.size() << ")";
        snapshot->truncated = true;
    }
    output << ":\n";
    for (const workspace_file_entry &file : snapshot->index.files)
    {
    ...
    for (const std::string &file : candidates)
    {
        ...
        {
            if (sampled_files < static_cast<uint64_t>(candidates.size()))
            {
                snapshot->truncated = true;
            }
            break;
        ...
        const size_t file_budget = clamp_to_size_t((std::min)(options.max_file_bytes, remaining));
        std::string content;
        bool file_truncated = false;
        if (!read_context_prefix(file, file_budget, &content, &file_truncated, nullptr))
        {
            continue;
        }
        if (content_looks_binary(content))
        {
            continue;
        }
        ...
        output << "\n--- file: " << relative_workspace_path(absolute, file) << " ---\n"
               << content;
        if (file_truncated)
        {
    ...
    snapshot->context_text = output.str();
    return true;
}
                                    ...
                                    std::string *error)
{
    ...
    workspace_context_snapshot snapshot;
    if (!build_workspace_context_snapshot(workspace_root, options, &snapshot, error))
    {
        return false;
    }

...
macro(dsn_setup_serialization)
    ...
    if (MY_PROJ_LANG STREQUAL "CXX")
        if (USE_THRIFT)
            if (DEFINED THRIFT_INCLUDE_DIR AND NOT "${THRIFT_INCLUDE_DIR}" STREQUAL "")
        ...
        if (USE_PROTOBUF)
            if (DEFINED PROTOBUF_INCLUDE_DIR AND NOT "${PROTOBUF_INCLUDE_DIR}" STREQUAL "")
                list(APPEND MY_PROJ_SYS_INC_PATH ${PROTOBUF_INCLUDE_DIR})
            endif ()
            if (DEFINED PROTOBUF_LIB_DIR AND NOT "${PROTOBUF_LIB_DIR}" STREQUAL "")
...
function(dsn_add_project)
    ...
    if(NOT DEFINED MY_PROJ_SYS_INC_PATH)
        set(MY_PROJ_SYS_INC_PATH "")
    endif()
    ...
    if((NOT DSN_GTEST_LIB_INDEX EQUAL -1) AND DEFINED GTEST_LIB_DIR AND NOT "${GTEST_LIB_DIR}" STREQUAL "")
        list(INSERT MY_PROJ_LIB_PATH 0 "${GTEST_LIB_DIR}/$<CONFIG>")
        list(FIND MY_PROJ_LIB_PATH "${GTEST_LIB_DIR}" DSN_GTEST_LIB_DIR_INDEX)
        if(DSN_GTEST_LIB_DIR_INDEX EQUAL -1)
    ...
    if(DEFINED DSN_DEBUG_CMAKE)
        ...
        message(STATUS "MY_PROJ_SYS_INC_PATH = ${MY_PROJ_SYS_INC_PATH}")
    ...
    if(NOT (MY_PROJ_SYS_INC_PATH STREQUAL ""))
    ...
    if(NOT DSN_GTEST_LIB_INDEX EQUAL -1)
        if(BUILD_TESTING)
...
macro(ms_setup_boost STATIC_LINK PACKAGES BOOST_LIBS)
    ...
    if(DSN_BOOST_HEADER_ONLY)
        if(DEFINED Boost_VERSION_MACRO)
            set(DSN_BOOST_VERSION_OK ${Boost_VERSION_MACRO})
            if(DSN_BOOST_VERSION_OK LESS 105300)
                message(FATAL_ERROR "Header-only Boost requires Boost 1.53 or newer")
            endif()
        elseif(DEFINED Boost_VERSION AND Boost_VERSION MATCHES "^[0-9]+$")
            if(Boost_VERSION LESS 105300)
                message(FATAL_ERROR "Header-only Boost requires Boost 1.53 or newer")
            endif()
        else()
            set(DSN_BOOST_VERSION_STRING ${Boost_VERSION})
            if(DEFINED Boost_VERSION_STRING)
                set(DSN_BOOST_VERSION_STRING ${Boost_VERSION_STRING})
            endif()
            if(DSN_BOOST_VERSION_STRING VERSION_LESS "1.53.0")
                message(FATAL_ERROR "Header-only Boost requires Boost 1.53 or newer")
            endif()
        endif()
    endif()
...
function(dsn_setup_boost_definitions)
    if(DSN_BOOST_HEADER_ONLY)
        add_definitions(-DBOOST_ERROR_CODE_HEADER_ONLY)
    endif()
endfunction(dsn_setup_boost_definitions)

function(dsn_setup_packages)
    dsn_setup_boost_definitions()
...
function(dsn_setup_include_path)
    ...
    elseif(DEFINED Boost_INCLUDE_DIRS AND NOT "${Boost_INCLUDE_DIRS}" STREQUAL "")
        include_directories(SYSTEM ${Boost_INCLUDE_DIRS})
    elseif(DEFINED Boost_INCLUDE_DIR AND NOT "${Boost_INCLUDE_DIR}" STREQUAL "")
        include_directories(SYSTEM ${Boost_INCLUDE_DIR})
    elseif(DEFINED BOOST_ROOT AND NOT "${BOOST_ROOT}" STREQUAL "")
...
function(dsn_setup_link_path)
    ...
    if(DSN_BUILD_RUNTIME)
        ...
        link_directories("${DSN_ROOT}/lib/$<CONFIG>" ${DSN_ROOT}/lib)
    endif()
...
if(NOT DSN_BUILD_RUNTIME)
    set(GTEST_INCLUDE_DIR "${DSN_ROOT}/include/ext")
    set(GTEST_LIB_DIR "${DSN_ROOT}/lib")
    if(BUILD_TESTING)
